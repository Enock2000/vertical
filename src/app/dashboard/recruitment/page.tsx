...
export default function RecruitmentPage() {
    ...
    useEffect(() => {
        ...
        const jobsUnsubscribe = onValue(jobsRef, (snapshot) => {
            ...
            // Update selected vacancy based on new list
            // We use the setter callback to access current selection without adding it to dependencies
            setSelectedVacancy(currentSelected => {
                if (!currentSelected && list.length > 0) {
                    // No current selection, default to first
                    return list[0];
                } else if (currentSelected) {
                    // Try to keep current selection
                    const updatedVacancy = list.find(v => v.id === currentSelected.id);
                    // If valid, use updated data; if deleted, fallback to first; if empty, null
                    return updatedVacancy || (list.length > 0 ? list[0] : null);
