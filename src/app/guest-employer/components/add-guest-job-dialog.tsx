// src/app/guest-employer/components/add-guest-job-dialog.tsx
'use client';
...
import { handleLoggedInGuestJobPosting } from '@/ai/flows/logged-in-guest-job-flow';
...
export function AddGuestJobDialog({ children }: { children: React.ReactNode }) {
  ...
  const onSubmit = async (values: GuestJobFormValues) => {
    if (!companyId) {
        toast({ variant: 'destructive', title: 'Error', description: 'Could not find your company account.' });
        return;
    }

    setIsLoading(true);
    try {
      const result = await handleLoggedInGuestJobPosting({
        ...values,
        companyId: companyId,
      ...
      if (result.success) {
        toast({
          title: 'Job Submitted',
          description: "Your new job posting is pending approval.",
        });
        form.reset();
        setOpen(false);
      } else {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="sm:max-w-2xl">
            <DialogHeader>
                ...
                <DialogDescription>
                    Fill out the form below to create a new job posting. All submissions are subject to review.
                </DialogDescription>
            </DialogHeader>
