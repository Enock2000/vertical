// src/app/guest-employer/layout.tsx
'use client';
...
const navItems = [
  { href: "/guest-employer", icon: LayoutDashboard, label: "Dashboard" },
  { href: "/careers", icon: Briefcase, label: "All Jobs" },
  { href: "/signup", icon: Zap, label: "Upgrade to Full" },
];

export default function GuestEmployerLayout({
  children,
}: {
  ...
  useEffect(() => {
    if (!loading) {
      if (!user) {
        router.push('/login');
      } else if (employee && employee.role !== 'GuestAdmin') {
        // Redirect to appropriate portal if not a guest admin
        router.push(employee.role === 'Admin' ? '/dashboard' : '/employee-portal');
      }
    }
