  ...
  async ({ sickNoteFile, ...requestData }) => {
    try {
      ...
      if (requestData.leaveType === 'Sick' && sickNoteFile && sickNoteFile.name) {
        const filePath = `sick-notes/${requestData.companyId}/${requestData.employeeId}/${Date.now()}-${sickNoteFile.name}`;
        const result = await uploadToB2(sickNoteFile, filePath);
        if (result.success && result.url) {
          newLeaveRequest.sickNoteUrl = result.url;
        }
      }
